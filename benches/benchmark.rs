...
use robodoan::sim::blockbuilding::{Block, BlockList};
use robodoan::sim::common::*;
...
// fn exec_moves_on_state(mut state: PuzzleState, twists: &[Twist]) ->
// PuzzleState {     for &twist in twists {
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let move_count = 32;
    let gen_random_moves = move || random_twists(&mut rng, move_count);

    let puzzle_with_2x2x2x2_block =
        BlockList::default().add_block_with_setup_moves(&[], Block::from_layer_bits(0xFF0));
